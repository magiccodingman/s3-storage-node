...
def test_real_sshfs_mount_and_durability_probe(tmp_path: Path) -> None:
    ...
    target = TargetConfig(
        ...
        sentinel_id="integration-dataset",
        allow_initialize=True,
        min_free_bytes=0,
        source=f"root@127.0.0.1:{remote}",
        transport_name="sshfs-integration",
        ssh_identity_file=str(client_key),
        ssh_known_hosts_file=str(known_hosts),
        ssh_runtime_identity_file=str(tmp_path / "runtime-key"),
        ssh_runtime_pid_file=str(tmp_path / "sshfs.pid"),
        ssh_port=port,
        mount_options=(
            f"IdentityFile={tmp_path / 'runtime-key'}",
    ...
    try:
        ...
        assert result["transport_name"] == "sshfs-integration"
