        ...
        for transport in self.transports:
            if transport.name == name:
                ...
                return replace(
                    ...
                    ssh_runtime_identity_file=str(runtime_key),
...
def _absolute_file(value: Any, name: str) -> str:
...
def load_exclusive_failover(config_path: str | os.PathLike[str], config: Config) -> ExclusiveFailoverConfig | None:
    ...
    cooldown = _integer(
        failover.get("failure_cooldown_seconds"),
        "storage.data.failover.failure_cooldown_seconds",
        60,
    )
    if cooldown <= 0:
    ...
    return ExclusiveFailoverConfig(
        ...
        worker_uid=config.appliance.uid,
        worker_gid=config.appliance.gid,
...
def resolve_target(
    ...
    if failover is None or target_name != failover.target:
        ...
        return base
    selected = transport_name or failover.primary_name
    return failover.resolve(base, selected)
...
class TransportSelector:
    ...
    def _load(self) -> dict[str, Any]:
        ...
        return {
            ...
            "failed": failed,
            "last_success_at": payload.get("last_success_at", 0.0),
        ...
        with self._locked():
            ...
            for name in self.config.ordered_names:
                ...
                if not isinstance(failure, dict):
                    ...
                    continue
                failed_at = float(failure.get("at", 0.0))
                available_at = failed_at + self.config.failure_cooldown_seconds
                if available_at <= now:
                    candidates.append(name)
                else:
    ...
    def record_failure(self, name: str, reason: str, now: float | None = None) -> None:
        ...
        with self._locked():
            state = self._load()
            failed = state["failed"]
            failed[name] = {"at": time.time() if now is None else now, "reason": reason[:1000]}
            state["active"] = name
            self._save(state)
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            **state,
            ...
            "ordered_transports": list(self.config.ordered_names),
            "failback_policy": self.config.failback_policy,
