...
Before installation, every live entry in the candidate index is checked against the authoritative `.dat` size using SeaweedFS's version-specific record sizing. Any entry extending beyond EOF is rejected without backing up or replacing the live index. After restart, each target must be present in `/status` with `ReadOnly=false`. A rejected or missing target causes another complete writer stop, restoration of that volume's original `.idx` and `.sdx`, and a persistent manual-intervention block. A complete record beyond the old index is normally recovered. An incomplete final record produces a manual-intervention state; the appliance never truncates the `.dat` to make it load.
...
If the bundled official `weed fix` reports success but its candidate contains a tail entry beyond authoritative `.dat` EOF, the pre-install bounds check rejects it and the guardian parks in `MANUAL_INTERVENTION_REQUIRED`. It stays alive for health and metric inspection but does not remount, rebuild, or restart SeaweedFS until the container is restarted after operator action. Do not retry with `-ignoreError` and do not truncate the `.dat` as an automatic recovery action. An operator may explicitly certify the volume as expected read-only only after verifying the retained data; document the reason next to `expected_readonly_volume_ids` in the deployment configuration.
