...
Before installation, every live entry in the candidate index is checked against the authoritative `.dat` size using SeaweedFS's version-specific record sizing. Any entry extending beyond EOF is rejected without replacing the live index. With `auto_tail_recovery_enabled=true`, exactly one proven incomplete final record may be recovered automatically: its present bytes are preserved and hash-verified locally, the unchanged source is truncated to that record's starting offset, and reconstruction restarts from the new fingerprint. Multiple violations, non-final violations, oversized tails, changed fingerprints, or an inconsistent live index always park for manual review. After restart, each target must be present in `/status` with `ReadOnly=false`.
...
If the bundled official `weed fix` candidate extends beyond EOF and the automatic-tail proof does not pass, the guardian parks in `MANUAL_INTERVENTION_REQUIRED`. It stays alive for inspection but does not remount or rebuild the unchanged source. Never retry with `-ignoreError`.
